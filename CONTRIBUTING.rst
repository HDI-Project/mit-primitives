...
MIT Primitives could always use more documentation, whether as part of the
official MIT Primitives docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `MIT Primitives` for local development.

1. Fork the `MIT Primitives` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/mit_primitives.git

3. Install your local copy into a virtualenv. Assuming you have virtualenvwrapper installed,
   this is how you set up your fork for local development::

    $ mkvirtualenv mit_primitives
    $ cd mit_primitives/
...
2. The tests that cover a module called ``mit_primitives/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/mit_primitives/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_mit_primitives
...
2. Update the version in ``setup.cfg``, ``mit_primitives/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``mit_primitives/__init__.py``
...
.. _GitHub issues page: https://github.com/HDI-Project/mit-primitives/issues
.. _Travis Build Status page: https://travis-ci.org/HDI-Project/mit-primitives/pull_requests
