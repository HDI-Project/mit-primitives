...
lint: ## check style with flake8 and isort
	flake8 mit_primitives tests
	isort -c --recursive mit_primitives tests
...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find mit_primitives -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive mit_primitives
	isort --apply --atomic --recursive mit_primitives
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source mit_primitives -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc --module-first --separate -o docs/api/ mit_primitives
	$(MAKE) -C docs html
